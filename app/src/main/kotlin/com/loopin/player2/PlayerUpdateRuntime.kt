...
class AuthenticatedPlayerUpdateSource(private val endpoint:String,private val credential:()->String?,private val installedVersionCode:Long,private val authoritativeChannel:(String)->Unit={}):PlayerUpdateSource,UpdateMediaSourceFactory {
    ...
    fun authorizeInstall(info:PlayerUpdateInfo):InstallAuthorizationResult { val secret=credential()?:return InstallAuthorizationResult.Unauthorized;val r=post(secret,JSONObject().put("action","authorize_install").put("release_id",info.releaseId).put("current_version_code",installedVersionCode));return when(r.first){in 200..299->InstallAuthorizationResult.Authorized;401,403,404,409->InstallAuthorizationResult.Unauthorized;else->InstallAuthorizationResult.Failed(r.first==null||r.first==408||r.first==429||(r.first?:0)>=500)} }
    fun reportInstall(attempt:UpdateInstallAttempt):Boolean{val secret=credential()?:return false;return post(secret,JSONObject().put("action","report_install").put("release_id",attempt.releaseId).put("state",attempt.state.name).put("failure_code",attempt.failureCode).put("current_version_code",BuildConfig.VERSION_CODE)).first in 200..299}

...
import { assignPlaylistVersion, createPlaylist, createReleaseUpload, createScreen, enqueuePlayerCommand, isReleaseAdmin, listHealthEvents, listMediaAssets, listPlayerReleases, listPlaylists, listPublishedPlaylistVersions, listRecentCommands, listScreens, listUpdateAttempts, login, logout, pairPlayer, publishPlaylistDraft, publishRelease, savePlaylistDraft, setDeviceUpdateChannel, uploadMediaAsset } from "./api";
...
import type { DeviceCommand, DeviceHealthEvent, DeviceUpdateAttempt, DraftItem, MediaAsset, PairingProof, PlayerCommandType, PlayerRelease, Playlist, PlaylistVersion, Screen, ScreenDevice } from "./types";
...
async function loadDashboard() {
  try {
    ...
    [screens, playlistVersions, mediaAssets, playlists, deviceCommands,healthEvents,updateAttempts] = await Promise.all([listScreens(supabase), listPublishedPlaylistVersions(supabase), listMediaAssets(supabase), listPlaylists(supabase), listRecentCommands(supabase),listHealthEvents(supabase),listUpdateAttempts(supabase)]);
...
async function refreshDashboardData() {
  if (!session) return;
  [screens,healthEvents,updateAttempts] = await Promise.all([listScreens(supabase),listHealthEvents(supabase),listUpdateAttempts(supabase)]);

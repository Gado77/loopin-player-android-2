...
import { activateUpdateRollout, assignPlaylistVersion, controlUpdateRollout, createPlaylist, createReleaseUpload, createScreen, createUpdateRollout, enqueuePlayerCommand, isReleaseAdmin, listHealthEvents, listMediaAssets, listPlayerReleases, listPlaylists, listPublishedPlaylistVersions, listRecentCommands, listScreenGroups, listScreens, listUpdateAttempts, listUpdateRollouts, login, logout, pairPlayer, publishPlaylistDraft, publishRelease, savePlaylistDraft, saveScreenGroup, setDeviceUpdateChannel, uploadMediaAsset } from "./api";
...
import type { DeviceCommand, DeviceHealthEvent, DeviceUpdateAttempt, DraftItem, MediaAsset, PairingProof, PlayerCommandType, PlayerRelease, Playlist, PlaylistVersion, Screen, ScreenDevice, ScreenGroup, UpdateRollout } from "./types";
...
import {canInstallUpdate,installStateLabel,installationCapabilityLabel,latestAttempt} from "./updates";
import {displayRolloutDevice,hasRolloutFailures,parseWaves,rolloutMetrics,rolloutPercent,validateWindow} from "./fleet";
...
let updateAttempts:DeviceUpdateAttempt[]=[];
let screenGroups:ScreenGroup[]=[];
let rollouts:UpdateRollout[]=[];
...
async function loadDashboard() {
  try {
    ...
    [screens, playlistVersions, mediaAssets, playlists, deviceCommands,healthEvents,updateAttempts,screenGroups] = await Promise.all([listScreens(supabase), listPublishedPlaylistVersions(supabase), listMediaAssets(supabase), listPlaylists(supabase), listRecentCommands(supabase),listHealthEvents(supabase),listUpdateAttempts(supabase),listScreenGroups(supabase)]);
    releases=releaseAdmin?await listPlayerReleases(supabase):[];
    rollouts=releaseAdmin?await listUpdateRollouts(supabase):[];
    renderDashboard();
  } catch (error) {
    renderDashboard();
    notice((error as Error).message);
...
async function refreshDashboardData() {
  if (!session) return;
  [screens,healthEvents,updateAttempts,screenGroups] = await Promise.all([listScreens(supabase),listHealthEvents(supabase),listUpdateAttempts(supabase),listScreenGroups(supabase)]);
  if(releaseAdmin)rollouts=await listUpdateRollouts(supabase);
...
function renderScreenCards() {
  return screens.map((screen) => {
    ...
    const attempt=latestAttempt(updateAttempts,screen.id);const install=`<div class="ota-status"><strong>${escape(installStateLabel(attempt?.state??runtime?.installation_state,runtime?.install_requested_at))}</strong><span>Versão atual: ${runtime?.current_version_code??"—"}${runtime?.prepared_version_code?` · Preparada: ${runtime.prepared_version_code}`:""}</span><small>${escape(installationCapabilityLabel(runtime?.installation_capability))}${attempt?.failure_code?` · ${escape(attempt.failure_code)}`:""}</small>${canInstallUpdate(runtime)?`<button class="command-button" data-screen-id="${screen.id}" data-command="INSTALL_UPDATE">Instalar atualização</button>`:""}</div>`;
    const actions=paired?`<section class="device-actions"><h4>Ações</h4><div><button class="secondary diagnostic-button" data-device-id="${device!.id}">Diagnóstico</button><button class="secondary command-button" data-screen-id="${screen.id}" data-command="GET_STATUS">Atualizar status</button><button class="secondary command-button" data-screen-id="${screen.id}" data-command="SYNC_NOW">Sincronizar agora</button><button class="secondary command-button" data-screen-id="${screen.id}" data-command="RELOAD_PLAYLIST">Recarregar playlist</button><button class="secondary command-button" data-screen-id="${screen.id}" data-command="CHECK_UPDATE">Verificar atualização</button></div><label>Canal<select class="update-channel" data-screen-id="${screen.id}"><option ${device?.update_channel!=="BETA"?"selected":""}>STABLE</option><option ${device?.update_channel==="BETA"?"selected":""}>BETA</option></select></label>${install}<div class="command-history" data-screen-id="${screen.id}">${renderCommandHistory(screen.id)}</div></section>`:"";
...
function renderDashboard() {
  ...
  const content = activeArea === "screens" ? renderScreensArea(cards) : activeArea === "media" ? renderMediaArea() : activeArea === "playlists" ? renderPlaylistsArea() : activeArea === "groups" ? renderGroupsArea() : renderUpdatesArea();
  root.innerHTML = `<header><div class="brand">LOOPIN <small>ADMIN 2.0</small></div>
    <nav class="main-nav"><button data-area="screens" class="text-button ${activeArea==="screens"?"active":""}">Telas</button><button data-area="media" class="text-button ${activeArea==="media"?"active":""}">Mídias</button><button data-area="playlists" class="text-button ${activeArea==="playlists"?"active":""}">Playlists</button><button data-area="groups" class="text-button ${activeArea==="groups"?"active":""}">Grupos</button>${releaseAdmin?`<button data-area="updates" class="text-button ${activeArea==="updates"?"active":""}">Atualizações</button>`:""}</nav>
  ...
  if(activeArea==="screens") bindScreensArea(); else if(activeArea==="media") bindMediaArea(); else if(activeArea==="playlists") bindPlaylistsArea(); else if(activeArea==="groups")bindGroupsArea();else bindUpdatesArea();
}
...
function renderGroupsArea(){const cards=screenGroups.map(g=>`<article class="content-card"><h3>${escape(g.name)}</h3><p>${g.members?.length??0} telas</p><small>${escape(g.description??"")}</small><button class="edit-group" data-id="${g.id}">Editar grupo</button></article>`).join("")||`<div class="empty">Nenhum grupo criado.</div>`;return`<div class="title-row"><div><h1>Grupos</h1><p>Organize telas reutilizáveis para operações de frota.</p></div><button id="new-group">Novo grupo</button></div><section class="content-grid">${cards}</section>`;}
function bindGroupsArea(){document.querySelector("#new-group")?.addEventListener("click",()=>renderGroupEditor(null));document.querySelectorAll<HTMLElement>(".edit-group").forEach(b=>b.addEventListener("click",()=>renderGroupEditor(screenGroups.find(g=>g.id===b.dataset.id)??null)));}
function renderGroupEditor(group:ScreenGroup|null){const selected=new Set(group?.members?.map(m=>m.screen_id));modal(`<form id="group-form"><div class="editor-heading"><div><h2>${group?"Editar":"Novo"} grupo</h2><p>Uma tela pode pertencer a vários grupos.</p></div></div><label>Nome<input name="name" maxlength="100" required value="${escape(group?.name??"")}"/></label><label>Descrição<textarea name="description" maxlength="500">${escape(group?.description??"")}</textarea></label><fieldset><legend>Telas</legend>${screens.map(s=>`<label class="check-row"><input type="checkbox" name="screens" value="${s.id}" ${selected.has(s.id)?"checked":""}/>${escape(s.name)}</label>`).join("")||"Nenhuma tela"}</fieldset><div class="actions"><button type="submit">Salvar</button><button type="button" class="secondary cancel">Cancelar</button></div></form>`);document.querySelector(".cancel")!.addEventListener("click",closeModal);document.querySelector<HTMLFormElement>("#group-form")!.addEventListener("submit",async e=>{e.preventDefault();const f=new FormData(e.currentTarget as HTMLFormElement);try{await saveScreenGroup(supabase,group?.id??null,String(f.get("name")),String(f.get("description")??""),f.getAll("screens").map(String));screenGroups=await listScreenGroups(supabase);closeModal();renderDashboard();notice("Grupo salvo.","success");}catch(error){notice((error as Error).message);}});}

function renderRolloutDevices(r:UpdateRollout){return(r.devices??[]).map(d=>{const s=screens.find(x=>x.id===d.screen_id),runtime=s?.devices?.[0]?.runtime_status,state=displayRolloutDevice(d,r,runtime);return`<article class="rollout-device" data-rollout-state="${state}"><strong>${escape(s?.name??"Tela snapshot")}</strong><span>${escape(state)}</span><small>${d.cohort_score===null?"Sem Player":`Cohort ${(d.cohort_score/100).toFixed(2)}%`}</small></article>`}).join("");}
function renderRolloutCard(r:UpdateRollout){const m=rolloutMetrics(r),percent=rolloutPercent(r),revoked=r.release?.status==="REVOKED",ready=(r.devices??[]).filter(d=>{const s=screens.find(x=>x.id===d.screen_id);return displayRolloutDevice(d,r,s?.devices?.[0]?.runtime_status)==="READY"}).length;return`<article class="rollout-card"><span class="status">${r.status}${revoked?" · RELEASE REVOKED":""}</span><h3>${escape(r.name)}</h3><p>v${r.release?.version_code??"—"} · Wave ${r.current_wave} · ${percent}%</p><div class="rollout-metrics"><span>${m.total}<small>total</small></span><span>${m.eligible}<small>elegíveis</small></span><span>${ready}<small>ready</small></span><span>${m.installed}<small>instalados</small></span><span>${m.failed}<small>falhas</small></span><span>${m.waiting}<small>próxima onda</small></span></div><small>${r.maintenance_timezone?`${escape(r.maintenance_timezone)} · ${r.maintenance_start_local} → ${r.maintenance_end_local}`:"Sem janela"}</small><div class="actions">${r.status==="DRAFT"?`<button class="rollout-action" data-id="${r.id}" data-action="ACTIVATE">Ativar</button>`:""}${r.status==="ACTIVE"?`<button class="rollout-action secondary" data-id="${r.id}" data-action="PAUSE">Pausar</button>${r.current_wave<r.waves.length?`<button class="rollout-action" data-id="${r.id}" data-action="ADVANCE">Avançar rollout</button>`:""}`:""}${r.status==="PAUSED"||r.status==="PAUSED_AUTO"?`<button class="rollout-action" data-id="${r.id}" data-action="RESUME">Retomar</button>`:""}${!["COMPLETED","CANCELED"].includes(r.status)?`<button class="rollout-action danger" data-id="${r.id}" data-action="CANCEL">Cancelar</button>`:""}</div><label>Filtrar devices<select class="rollout-filter"><option value="ALL">Todos</option><option value="PENDING">Pendentes</option><option value="READY">Ready</option><option value="INSTALLED">Instalados</option><option value="FAILED">Falhas</option></select></label><div class="rollout-devices">${renderRolloutDevices(r)}</div></article>`;}
function renderUpdatesArea(){const cards=releases.map(r=>`<article class="content-card"><span class="status">${r.channel}</span><h3>${r.version_code?`v${r.version_code} · ${escape(r.version_name??"")}`:"Aguardando inspeção"}</h3><p>${(r.apk_size_bytes/1048576).toFixed(2)} MiB · ${r.apk_sha256?r.apk_sha256.slice(0,12)+"…":"SHA pendente"}</p><small>${r.status} · ${new Date(r.published_at??r.created_at).toLocaleString("pt-BR")}</small><div class="actions">${r.status==="DRAFT"&&r.inspected_at?`<button class="publish-release" data-id="${r.id}">Publicar</button>`:""}${r.status==="PUBLISHED"?`<button class="create-rollout" data-id="${r.id}">Criar rollout</button><button class="secondary revoke-release" data-id="${r.id}">Revogar</button>`:""}</div></article>`).join("")||`<div class="empty">Nenhum release.</div>`;return `<div class="title-row"><div><h1>Atualizações</h1><p>Publicar habilita o artefato; somente um rollout ativo distribui.</p></div><button id="new-release">Novo release</button></div><input id="release-file" type="file" accept=".apk,application/vnd.android.package-archive" hidden/><label>Canal<select id="release-channel"><option>STABLE</option><option>BETA</option></select></label><label>Notas<textarea id="release-notes" maxlength="4000"></textarea></label><div id="upload-state" class="upload-state"></div><section class="content-grid">${cards}</section><h2>Rollouts</h2><section class="rollout-list">${rollouts.map(renderRolloutCard).join("")||`<div class="empty">Nenhum rollout.</div>`}</section>`;}
function renderRolloutEditor(releaseId:string){modal(`<form id="rollout-form"><div class="editor-heading"><div><h2>Criar rollout</h2><p>Targets ficam imutáveis após ativação.</p></div></div><label>Nome<input name="name" maxlength="120" required/></label><fieldset><legend>Grupos</legend>${screenGroups.map(g=>`<label class="check-row"><input type="checkbox" name="groups" value="${g.id}"/>${escape(g.name)} · ${g.members?.length??0}</label>`).join("")||"Nenhum grupo"}</fieldset><fieldset><legend>Telas individuais</legend>${screens.map(s=>`<label class="check-row"><input type="checkbox" name="screens" value="${s.id}"/>${escape(s.name)}</label>`).join("")}</fieldset><label>Ondas (%)<input name="waves" value="5, 25, 100" required/></label><label>Início agendado (opcional)<input name="scheduled" type="datetime-local"/></label><div class="coordinate-grid"><label>Timezone IANA<input name="timezone" placeholder="America/Fortaleza"/></label><label>De<input name="start" type="time"/></label><label>Até<input name="end" type="time"/></label></div><div class="actions"><button type="submit">Criar rollout</button><button type="button" class="secondary cancel">Cancelar</button></div></form>`);document.querySelector(".cancel")!.addEventListener("click",closeModal);document.querySelector<HTMLFormElement>("#rollout-form")!.addEventListener("submit",async e=>{e.preventDefault();const f=new FormData(e.currentTarget as HTMLFormElement);try{const window=validateWindow(String(f.get("timezone")??""),String(f.get("start")??""),String(f.get("end")??""));await createUpdateRollout(supabase,{releaseId,name:String(f.get("name")),groupIds:f.getAll("groups").map(String),screenIds:f.getAll("screens").map(String),waves:parseWaves(String(f.get("waves"))),scheduledStartAt:f.get("scheduled")?new Date(String(f.get("scheduled"))).toISOString():null,timezone:window?.timezone??null,windowStart:window?.start??null,windowEnd:window?.end??null});rollouts=await listUpdateRollouts(supabase);closeModal();renderDashboard();notice("Rollout DRAFT criado.","success");}catch(error){notice((error as Error).message);}});}
function bindUpdatesArea(){document.querySelector("#new-release")?.addEventListener("click",()=>document.querySelector<HTMLInputElement>("#release-file")!.click());document.querySelector<HTMLInputElement>("#release-file")?.addEventListener("change",async e=>{const input=e.currentTarget as HTMLInputElement,file=input.files?.[0];if(!file)return;const state=document.querySelector<HTMLElement>("#upload-state")!;try{await createReleaseUpload(supabase,file,(document.querySelector<HTMLSelectElement>("#release-channel")!.value as "STABLE"|"BETA"),document.querySelector<HTMLTextAreaElement>("#release-notes")!.value,s=>state.textContent=s);releases=await listPlayerReleases(supabase);renderDashboard();notice("APK privado enviado. Execute a inspeção controlada antes de publicar.","success");}catch(error){notice((error as Error).message);}finally{input.value="";}});document.querySelectorAll<HTMLButtonElement>(".publish-release,.revoke-release").forEach(b=>b.addEventListener("click",async()=>{try{await publishRelease(supabase,b.dataset.id!,b.classList.contains("revoke-release"));releases=await listPlayerReleases(supabase);renderDashboard();notice("Release atualizado.","success");}catch(error){notice((error as Error).message);}}));document.querySelectorAll<HTMLButtonElement>(".create-rollout").forEach(b=>b.addEventListener("click",()=>renderRolloutEditor(b.dataset.id!)));document.querySelectorAll<HTMLButtonElement>(".rollout-action").forEach(b=>b.addEventListener("click",async()=>{const r=rollouts.find(x=>x.id===b.dataset.id)!,action=b.dataset.action!;let prompt=action==="ADVANCE"?`Avançar de ${rolloutPercent(r)}% para ${r.waves[r.current_wave]}%?`:action==="CANCEL"?"Cancelar definitivamente este rollout?":`${action} rollout?`;if(action==="ADVANCE"&&hasRolloutFailures(r))prompt+=`\n${rolloutMetrics(r).failed} falha(s) detectada(s).`;if(!confirm(prompt))return;try{if(action==="ACTIVATE")await activateUpdateRollout(supabase,r.id);else await controlUpdateRollout(supabase,r.id,action as "PAUSE"|"RESUME"|"ADVANCE"|"CANCEL");rollouts=await listUpdateRollouts(supabase);renderDashboard();notice("Rollout atualizado.","success");}catch(error){notice((error as Error).message);}}));document.querySelectorAll<HTMLSelectElement>(".rollout-filter").forEach(select=>select.addEventListener("change",()=>select.closest(".rollout-card")!.querySelectorAll<HTMLElement>(".rollout-device").forEach(row=>row.hidden=select.value!=="ALL"&&row.dataset.rolloutState!==select.value)));}

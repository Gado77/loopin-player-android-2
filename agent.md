...
# Estado atual — Fase 15 (2026-09-05)

Release `PUBLISHED` não é mais distribuído globalmente. O backend somente oferece update a devices presentes no snapshot de um rollout `ACTIVE`, dentro da onda e janela de manutenção, no canal correto e com release ainda publicado. Grupos de telas genéricos e many-to-many são tenant-safe e reutilizáveis; seus membros são copiados e deduplicados na ativação, portanto alterações futuras no grupo não alteram uma operação em andamento.

Rollouts suportam `DRAFT`, `SCHEDULED`, `ACTIVE`, `PAUSED`, `PAUSED_AUTO`, `COMPLETED` e `CANCELED`, ondas determinísticas padrão 5/25/100, conflito entre operações concorrentes, janela IANA inclusive atravessando meia-noite e circuit breaker após pelo menos cinco tentativas com 30% de falha. O Admin 2 ganhou as áreas Grupos e gestão mínima de rollout. A Edge Function decide elegibilidade; o Android permanece na Fase 14, sem scheduler, serviço ou dependência nova.

As migrations e funções foram implantadas no Supabase isolado. Passaram 229 testes Android/JVM, 101 testes Admin, lint, builds debug/release, build web e audit sem vulnerabilidades. O LDPlayer preservou pairing/stores e iniciou manualmente sem crash após reboot, mas a autoabertura por `BOOT_COMPLETED` não foi observada em até 40 segundos nesta rodada e deve ser revalidada. O E2E binário v14→v15, assinatura LAB e certificação MXQ continuam pendentes. Documento autoritativo: `docs/PHASE15_FLEET_OTA_ROLLOUT.md`.

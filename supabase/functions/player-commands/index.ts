...
function safeResult(commandType:string,value:unknown){if(!value||typeof value!=="object"||Array.isArray(value))return null;const source=value as Record<string,unknown>;if(JSON.stringify(source).length>4096)return null;if(commandType==="GET_STATUS"){const keys=["app_version","connection","playback_state","cache_state","sync_state","health_state","free_storage_bytes","last_sync_epoch_ms"];if(!exact(source,keys)||typeof source.app_version!=="string"||source.app_version.length>100||typeof source.free_storage_bytes!=="number"||!Number.isSafeInteger(source.free_storage_bytes)||source.free_storage_bytes<0||!(source.last_sync_epoch_ms===null||(typeof source.last_sync_epoch_ms==="number"&&Number.isSafeInteger(source.last_sync_epoch_ms)&&source.last_sync_epoch_ms>=0))||keys.slice(1,6).some(k=>typeof source[k]!=="string"||String(source[k]).length>32))return null;return source;}if(!exact(source,["code"])||typeof source.code!=="string"||source.code.length<1||source.code.length>64)return null;return{code:source.code};}
